...
rHandPhoto = PhotoImage(file='RockPaperScissorsImages/rHand.png')  # Adjust path if necessary
pHandPhoto = PhotoImage(file='RockPaperScissorsImages/pHand.png')  # Adjust path if necessary
sHandPhoto = PhotoImage(file='RockPaperScissorsImages/sHand.png')  # Adjust path if necessary

# Load game images from RockPaperScissorsImages folder
...
def updateLeaderboard():
    global userWin, compWin, counter, turns
    
    # Get user stats
    games_played = counter
    games_won = userWin
    games_lost = compWin
    games_tied = turns - games_played - games_won - games_lost
    winning_rate = (games_won / games_played) * 100 if games_played > 0 else 0
    
    conn = sqlite3.connect('LeaderBoard.db')
    c = conn.cursor()
    
    # Check if the player already exists in the leaderboard
    c.execute("SELECT * FROM LEADERBOARD WHERE name=?", ('Shruthika',))  # Replace 'Shruthika' with dynamic player name
    ...
    if existing_player:
        ...
        c.execute('''
            UPDATE LEADERBOARD
            SET games_won = ?, games_lost = ?, games_tied = ?, games_played = ?, winning_rate = ?
            WHERE name = ?
        ''', (games_won, games_lost, games_tied, games_played, winning_rate, 'Shruthika'))
    else:
        # Insert new player's data
        c.execute('''
            INSERT INTO LEADERBOARD (name, games_won, games_lost, games_tied, games_played, winning_rate)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', ('Shruthika', games_won, games_lost, games_tied, games_played, winning_rate))
...
# Initialize the LeaderBoard database and table
conn = sqlite3.connect('LeaderBoard.db')
c = conn.cursor()
c.execute('''
...
# Now, you can call the getLeaderboard() function
getLeaderboard()
def play():
